...
?>
                    ...
                    <div class="span12">
                        ...
                        <div class="span1">
                            <div>
                                ...
                                <input type="text" class="span11" name="qty" id="qty" autocomplete="off" onkeyup="genetate_total(this.value)">
                            </div>
                        </div>
    ...
    function add_session(){
        ...
        if (xmlhttp.readyState==4 && xmlhttp.status==200){
           if(xmlhttp.responseText==""){
            ...
            load_billing_products();
            location.reload("http://localhost/php_ims/admin/sales_master.php");
           }
   ...
   function load_total_bill(){
      ...
      xmlhttp.open("GET","forajax/load_billing_amounts.php" ,true);
      xmlhttp.send();

    }
    load_billing_products();

    function edit_qty(qty1,company_name1,product_name1,unit1,packing_size1,prize1){
...
<?php
    ...
    if(isset($_POST['submit1'])){
      $lastbillno=0;
      mysqli_query($conn,"insert into billing_header values(NULL,'$_POST[full_name]','$_POST[bill_type_header]','$_POST[Date]','$_POST[bill_no]','$_SESSION[admin]')")  or die(mysqli_error($conn));
      ...
      for($i=0; $i < $max; $i++){
        ...
        if(isset($_SESSION['cart'][$i])){
           ...
           mysqli_query($conn,"insert into billing_details values(NULL,'$lastbillno','$company_name_session','$product_name_session','$unit_session','$packing_size_session','$price_session','$qty_session')") or die(mysqli_error($conn));
           mysqli_query($conn,"update stock_master set product_qty=product_qty-$qty_session where product_company='$company_name_session' && product_name='$product_name_session' && product_unit='$unit_session'");
            }
