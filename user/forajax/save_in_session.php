...
if(isset($_SESSION['cart'])){
    ...
    if($check_available==0){
        $available_qty=check_qty($company_name,$product_name,$unit,$packing_size,$conn);
        ...
        $av_qty=check_qty($company_name,$product_name,$unit,$packing_size,$conn);
        if($av_qty>=$exist_qty){
...
function check_qty($product_company,$product_name,$product_unit,$packing_size,$conn){
    $product_qty=0;
    $res=mysqli_query($conn,"select * from stock_master where product_company='$product_company' && product_name='$product_name' && product_unit='$product_unit'");
